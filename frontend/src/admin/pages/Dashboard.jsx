...
const Dashboard = () => {
  const [stats, setStats] = useState({ enquiries: 0, applications: 0, news: 0, jobs: 0 });
  ...
  const getStatusBadge = (status) => {
    const classes = { New: 'badge-new', 'In Progress': 'badge-draft', Replied: 'badge-active', Archived: 'badge-archived' };
    ...
    <>
      ...
      <div className="admin-content">
        <div style={{ marginBottom: '32px' }}>
          <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '4px' }}>{getGreeting()}, Admin</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '24px' }}>
          <div className="card">
            ...
            {loading ? (
              <div style={{ textAlign: 'center', padding: '40px', color: 'var(--admin-text-muted)' }}>Loading...</div>
            ) : recentEnquiries.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px' }}><Mail size={48} style={{ color: 'var(--admin-text-light)', marginBottom: '16px' }} /><p style={{ color: 'var(--admin-text-muted)' }}>No enquiries yet.</p></div>
            ) : (
              <>
                <div className="table-container">
                  <table className="table"><thead><tr><th>Name</th><th>Company</th><th>Type</th><th>Date</th><th>Status</th></tr></thead><tbody>
          ...
          <div className="card">
            <div className="card-header"><h2 className="card-title">Quick Actions</h2><p className="card-subtitle">Common tasks</p></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <Link to="/admin/cms/news" className="btn btn-secondary" style={{ justifyContent: 'flex-start' }}><Plus size={16} /> Add Announcement</Link>
              <Link to="/admin/cms/products" className="btn btn-secondary" style={{ justifyContent: 'flex-start' }}><Package size={16} /> Update Product Spec</Link>
              <Link to="/admin/cms/gallery" className="btn btn-secondary" style={{ justifyContent: 'flex-start' }}><Image size={16} /> Upload Gallery Photos</Link>
              <Link to="/admin/cms/careers" className="btn btn-secondary" style={{ justifyContent: 'flex-start' }}><Briefcase size={16} /> Post a New Job</Link>
            </div>
          </div>
        </div>
      </div>
...
const KPICard = ({ icon, iconColor, label, value, trend }) => (
  <div className="card" style={{ padding: '20px' }}>
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--admin-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>{label}</div>
        <div style={{ fontSize: '32px', fontWeight: 800, fontFamily: 'var(--font-primary)', marginBottom: '4px' }}>{value.toLocaleString()}</div>
        <div style={{ fontSize: '12px', color: 'var(--admin-green)' }}>{trend}</div>
      </div>
    </div>
  </div>
);
