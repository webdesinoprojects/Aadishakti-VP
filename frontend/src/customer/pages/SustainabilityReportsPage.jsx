...
export default function SustainabilityReportsPage() {
  ...
  const currentItems = (data.sustainability || []).slice(indexOfFirst, indexOfLast);

  return (
    <div style={{ padding: '40px' }}>
      <CustomerPageHeader title="Sustainability Reports (COA)" subtitle="Download Reports of Analysis for your products." />
      <div className="customer-card" style={{ padding: 0 }}>
        <table className="customer-table">
          <thead>
            <tr><th>QC ID</th><th>Batch No.</th><th>Product</th><th>Date</th><th>Status</th><th>Report</th></tr>
          </thead>
          <tbody>
            {currentItems.map(q => (
            ...
            {currentItems.length === 0 && <tr><td colSpan="6" style={{ textAlign: 'center' }}>No reports found.</td></tr>}
          </tbody>
        </table>
        {data.sustainability?.length > 0 && <CustomerPagination currentPage={currentPage} totalPages={Math.ceil(data.sustainability.length / itemsPerPage)} onPageChange={setCurrentPage} totalItems={data.sustainability.length} itemsPerPage={itemsPerPage} />}
      </div>
