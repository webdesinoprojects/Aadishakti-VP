...
export default function ReconciliationPage() {
  const user = { username: 'c1' };
  const [soas, setSoas] = useState([
    { id: 2, userId: 'C-3029', role: 'Customer', quarter: 'Q1 2026', fileName: 'soa_c1.pdf', originalName: 'Statement_Q1_2026.pdf', status: 'Verified', createdAt: new Date(Date.now() - 500000000).toISOString() },
    { id: 4, userId: 'C-3029', role: 'Customer', quarter: 'Q4 2025', fileName: 'soa_c2.pdf', originalName: 'Statement_Q4_2025.pdf', status: 'Verified', createdAt: new Date(Date.now() - 8000000000).toISOString() }
  ]);
  ...
  const handleUpload = async (e) => {
    ...
    setUploading(true);
    setTimeout(() => {
      setSoas([{ id: Date.now(), userId: user.username, role: 'Customer', quarter, fileName: file.name, originalName: file.name, status: 'Pending Verification', createdAt: new Date().toISOString() }, ...soas]);
      setFile(null);
      setUploading(false);
    }, 1000);
  };
